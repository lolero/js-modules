...
- **Naming**: order identifier parts so the grouping axis comes first — favor sorting and autocomplete over grammatical reading order. A **category** leads (`cssSelectorButtonDisabled`, `commandTscEmit`, `configBabelReactLib`), as do verbs and predicates (`getX`, `isX`, `useX`). A **type or role** trails, reading as an annotation on a domain-led name (`navLeftDrawerTreeViewItemLinkSx`, `WorkspaceContentBoxProps`). When both could apply, cluster on the domain.
...
- **MUI `sx` — how to write it**: keep it statically analyzable — plain strings/numbers and system shorthands (`pr: 1.5`, `color: 'primary.main'`), never inline computation or per-property callbacks (`pr: (t) => t.spacing(1.5)`). Note `pr`/`px`/`my` take spacing multipliers, but `width`/`height` are sizing (`0.25` means `25%`) and `top`/`right`/`bottom`/`left` are raw CSS — those need `theme.spacing()` from a hoisted `useTheme()`. Write `sx` inline and readable — the React Compiler caches the whole element, so hoisting a static `sx` to a `const` inside a component buys nothing, including in list and row renderers. Extract only for reuse (a shared object used at more than one call site) or in plain non-component code, which the compiler never touches.
