...
{#if $errorStore.errors.length}
  <div
    on:click={reset}
    on:keypress={reset}
    class="fixed z-10 inset-0 bg-gray-500 opacity-40"
  />
  <div
    class="fixed modal z-20 top-0 right-0 bottom-0 w-6/12 bg-red-300 p-3 flex flex-col gap-1"
  >
    ...
    <div class="flex-1 bg-white overflow-y-scroll">
      {#each $errorStore.errors as error, i (i)}
