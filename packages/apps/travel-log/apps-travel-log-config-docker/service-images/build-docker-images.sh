...
prompt_credentials() {
    ...
    # Prompt for username
    read -p "Docker Hub Username: " username_input

    # Prompt for password (hidden)
    read -s -p "Docker Hub Password: " password_input
    echo ""
    echo ""
...
get_latest_version() {
    ...
    local token=$(get_docker_hub_token)
    ...
    local versions=$(echo "$tags" | grep -o '"name":"v[0-9]\+\.[0-9]\+\.[0-9]\+"' | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' || echo "")
    ...
    local latest=$(echo "$versions" | sed 's/v//' | sort -V | tail -1)
    echo "$latest"
}
...
prompt_version_update() {
    ...
    local new_version=$(increment_version "$current_version" "${VERSION_UPDATES[$service]}")
...
main() {
    ...
    if prompt_dev_mode; then
        ...
        if check_docker_auth; then
            local current_user=$(get_docker_username)
            ...
            while true; do
                read -p "Continue as $current_user? (y/n): " choice
                case $choice in
                    [Yy])
                        ...
                        echo ""
                        read -s -p "Enter Docker Hub password (for API access to fetch versions): " password_input
                        echo ""
                        DOCKER_PASSWORD="$password_input"
    ...
    if [ "$AUTHENTICATED" = true ]; then
        echo ""
        read -p "Logout from Docker Hub? (y/n): " logout_choice
