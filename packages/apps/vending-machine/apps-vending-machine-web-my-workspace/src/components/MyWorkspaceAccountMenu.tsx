...
export const MyWorkspaceAccountMenu: React.FC = () => {
  ...
  const balanceString = useMemo(() => {
    return myBalance === null || myBalance === undefined
      ? '$0'
      : `$${myBalance}`;
  }, [myBalance]);
  ...
  return (
    <>
      <Tooltip title={username}>
        <Badge
          overlap="circular"
          badgeContent={balanceString}
          color={myBalance && myBalance > 0 ? 'success' : 'error'}
        >
      ...
      >
        ...
        <MenuItem disabled>
          <ListItemText>Balance: {balanceString}</ListItemText>
        </MenuItem>
