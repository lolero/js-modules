...
function randomBool(likelihood: number): boolean {
...
export const resources: SchedulerResource[] = agendaResources.map((r) => ({
  id: r.id,
  title: r.title,
...
const RECURRING_EVENTS: SchedulerEvent[] = [
  {
    id: 'rec-standup',
    title: 'Daily Standup',
...
const WEIGHTED_TEMPLATES: TitleTemplate[] = TITLE_TEMPLATES.flatMap((t) =>
  Array.from({ length: t.weight }, () => t),
);

const DURATIONS_MINUTES = [30, 45, 60, 90, 120, 180] as const;

function generateOneTimeEvents(): SchedulerEvent[] {
...
const ALL_EVENTS: SchedulerEvent[] = [...RECURRING_EVENTS, ...generateOneTimeEvents()];
 ...
 */
function estimateRecurrenceEnd(event: SchedulerEvent): Date | null {
