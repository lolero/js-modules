...
interface ClipboardPasteHistoryData {
  oldRows: Map<GridRowId, Record<string, any>>;
  newRows: Map<GridRowId, Record<string, any>>;
}

export function createCustomCellEditHandler(
  ...
  return {
    ...
    validate: (data: CellEditHistoryData, direction: 'undo' | 'redo') => {
...
export function createCustomClipboardPasteHistoryHandler(
  apiRef: React.RefObject<GridApiPremium>,
): GridHistoryEventHandler<ClipboardPasteHistoryData> {
  return {
    store: (params: ClipboardPasteHistoryData) => params,
    validate: (data: ClipboardPasteHistoryData, direction: 'undo' | 'redo') => {
      ...
      let expectedPage: number | null = null;
    ...
    undo: async (data: ClipboardPasteHistoryData) => {
      const { oldRows, newRows } = data;
      ...
      if (oldRowsValues.length > 0 && columnOrder.length > 0) {
        ...
        const rowIndex = gridExpandedSortedRowIndexLookupSelector(apiRef)[firstNewRowId];
        const paginationModel = gridPaginationModelSelector(apiRef);
        if (paginationModel && rowIndex !== undefined) {
          const page = Math.floor(rowIndex / paginationModel.pageSize);
          if (page !== paginationModel.page) {
    ...
    redo: async (data: ClipboardPasteHistoryData) => {
      const { oldRows, newRows } = data;
