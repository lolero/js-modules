...
function getFlatStockRows(tick: number): StockRow[] {
  const rows: StockRow[] = [];
...
function getGroupingValue(row: StockRow, field: string) {
  return String(row[field] ?? '');
}

function getGroupRowId(
  groupFields: string[],
  groupKeys: string[],
  groupValue: string,
) {
...
function getGroupRowsChildrenCount(
  rows: StockRow[],
  groupFields: string[],
  depth: number,
) {
...
function getGroupedStockRows(
  ...
  const groupedRows = new Map<string, StockRow[]>();
...
function fakeRowGroupingStockServer(
