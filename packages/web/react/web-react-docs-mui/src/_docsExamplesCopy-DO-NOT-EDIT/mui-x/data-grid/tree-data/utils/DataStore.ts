...
interface RowWithPath extends RowData {
  path: string[];
}

interface StoredData {
  rowLookup: Record<GridRowId, RowData>;
  tree: TreeNode[];
}
...
export class DataStore {
  ...
  private loadFromLocalStorage(): StoredData | null {
    try {
  ...
  private computeRowsWithPaths(): RowWithPath[] {
    const result: RowWithPath[] = [];

    const traverse = (nodeId: GridRowId, parentPath: string[]) => {
